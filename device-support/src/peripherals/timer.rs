...
pub struct GenericTimers<S, T, U, V>
where
    ...
    S: Into<u16> + From<u16> + Into<V>,
{
    ...
    interrupt_flags: TimerArr<AtomicBool>,
...
impl<S, T, U, V> GenericTimers<S, T, U, V>
where
    T: CountDown<Time = V> + Periodic,
    U: CountDown<Time = V> + Periodic,
    S: Into<u16> + From<u16> + Into<V>,
{
    pub fn new(hal_timer0: T, hal_timer1: U) -> Self {
        Self {
            ...
            states: RefCell::new(TimerArr(
                [TimerState::Disabled; TimerId::NUM_TIMERS],
            )),
            modes: TimerArr([TimerMode::SingleShot; TimerId::NUM_TIMERS]),
            interrupt_flags: TimerArr([
                AtomicBool::new(false),
                AtomicBool::new(false),
            ]), // TODO: remove these, they're not actually used (just the non-blocking API in embedded-hal)
            phantom: PhantomData,
        }
    }
}
            ...
            TimerState::WithPeriod(_) => {
                match $hal_timer.wait() {
                    Ok(()) => {
                        ...
                        $self.interrupt_flags[$timer_id].store(true, core::sync::atomic::Ordering::SeqCst);// = AtomicBool::new(true);
                    }
                    _=> {} // TODO: check for `WouldBlock`
                           // TODO: offer an impl that does the traditional interrupt-flag-in-ISR style synchronization
                }
            }
        }
...
impl<S, T, U, V> Timers for GenericTimers<S, T, U, V>
where
    T: CountDown<Time = V> + Periodic,
    U: CountDown<Time = V> + Periodic,
    S: Into<u16> + From<u16> + Into<V>,
{
    ...
    fn get_states(&self) -> TimerArr<TimerState> {
        ...
        TIMERS.iter().for_each(|t| states[*t] = self.get_state(*t));
    ...
    fn interrupts_enabled(&self, timer: TimerId) -> bool {
        matches!(self.get_state(timer), TimerState::WithPeriod(_))
            || (self.get_state(timer) == TimerState::Disabled
                && self.interrupt_occurred(timer))
    }
}
