    ...
    S: EventFutureSharedStatePorcelain,
{
    ...
    // TODO: explain better, esp the potential downsides (repeated messages!)
    //
    // If enabled, retries sending the request messages for a response we're
    // waiting on after the given timeout.
    //
    // This can lead to the device getting duplicated request messages; for
    // commands that are not idempotent (i.e. Step) this will be observable.
    //
    // The upside is that this enables the `Controller` to "recover" from errors
    // like the board being restarted in a way that's not aligned with req/resps.
    #[cfg(not(feature = "no_std"))]
    ...
    S: EventFutureSharedStatePorcelain,
{
    ...
    //
    // In situations where this is called after sending data and a response is expected
    // `use_blocking_get` can be set to `true`. This will use `get_blocking` on
    // the underlying transport which may save the caller of this function (if waiting
    // on a response) some spinning.
            ...
            match Controller::tick_inner($s, true) {
                ...
                Ok(m) => {
                    if let $resp = m {
                        ...
                        //
                        // We're effectively throwing this response message away and trying again (if we time out).
                        // The hope is that we'll eventually realign ourselves.
                        //
                        // The ideal case is just that the device went away for a bit (i.e. was paused in GDB)
                        // causing us to retry meaning that we're now getting duplicated respoonses for the
                        // previous request we sent out. In the future maybe we'll cache the last request we
                        // tried and _ensure_ that the response type we're getting unexpectedly is that of the
                        // previous message.
                        //
                ...
                Err(None) => {
                    ...
                    #[cfg(not(feature = "no_std"))]
                    {
                        // Try again!
                        //
                        // Hopefully the message actually was lost... otherwise we'll get two responses
                        // and crash.
                        ...
                        // TODO: we should maybe "poison" the run until event future here, if we're
                        // assuming that the board has been reset...
                        //
                        // Or not; I think we don't intend to support tolerating board resets in the
                        // general case. If there are no pending futures at the time of reset it'll
                        // __happen__ to work and that's okay.
                        //
                        // Note that if we ever do a caching control middleware, it'll have a lot of
                        // its assumptions violated if the board resets from under it. That's okay
                        // too.
                    }
                },
                ...
                // and try again. (TODO: this is... fraught. but given how rare UART
                // transmission errors are, this is probably good enough)
    ...
    S: EventFutureSharedStatePorcelain,
{
    ...
    fn get_adc_readings(&self) -> AdcPinArr<Result<u16, AdcReadError>> { ctrl!(self, GetAdcReadings, R::GetAdcReadings(r), r) }
