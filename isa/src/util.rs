...
impl MemoryDump {
    ...
    pub fn layer_loadable<L: LoadableIterator>(
        &mut self,
        loadable: L,
    ) -> &mut Self {
    ...
    pub fn layer_iterator<I: Iterator<Item = (Addr, Word)>>(
        &mut self,
        iter: I,
    ) -> &mut Self {
...
impl From<AssembledProgram> for MemoryDump {
    fn from(memory: AssembledProgram) -> Self {
        let mut mem: [Word; ADDR_SPACE_SIZE_IN_WORDS] =
...
impl<'a> IntoIterator for &'a MemoryDump {
    ...
    type IntoIter = Map<
        Enumerate<Iter<'a, Word>>,
        &'a dyn Fn((usize, &Word)) -> (Addr, Word),
    >;
...
impl<'a> IntoIterator for &'a AssembledProgram {
    ...
    fn into_iter(self) -> Self::IntoIter {
        self.iter()
            ...
            .map(&|(idx, (word, _)): (usize, &(Word, bool))| {
                (idx as Addr, *word)
            })
    }
}
